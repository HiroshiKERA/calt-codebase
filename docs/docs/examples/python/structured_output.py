...
def basic_example():
    ...
    result = agent.structured_output(
        PersonInfo, "John Smith is a 30-year-old software engineer"
    )
...
def conversation_history_example():
    ...
    result = agent.structured_output(
        CityInfo, "Extract structured information about Paris"
    )
...
def complex_nested_model_example():
    ...
    class Person(BaseModel):
        ...
        skills: List[str] = Field(
            default_factory=list, description="Professional skills"
        )
